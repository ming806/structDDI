...
if __name__ == "__main__":
    ...
    # (83040, 5)
    biosnap_smiles_list = load_decompressed_pzip("../biosnap_raw_index_smiles_list.pzip")
    num = len(biosnap_smiles_list)
    print(len(biosnap_smiles_list))  # 9651
    file_path = "../biosnap_raw_index_smiles_list.pzip"
    # 01 将一维的smiles转换为二维的文本信息(graph_num,text_len)
    output = encoding_smile_sequence_from_list(file_path, max_len=100)
    store_compressed_pzip(output, "../biosnap_raw_index_text_array.pzip")
    ...
    store_compressed_pzip(adj_list, "../biosnap_raw_index_adj.pzip")        # 邻接矩阵，因为每个图的矩阵大小不唯一，所以无法用h5进行存储
    store_compressed_pzip(feat_list, "../biosnap_raw_index_feat.pzip")      # 特征矩阵
