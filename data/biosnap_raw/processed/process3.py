...
if __name__ == "__main__":
    df = pd.read_csv("../biosnap_interaction_raw.csv")  # (83040, 5)
    ...
    out_df.to_csv("../biosnap_raw_index.csv", index=False)
    print(out_df.shape)    # (9651, 2)  按照smile字符串从短到长进行排列，然后生成该数据唯一的节点id。
    # h5_object = [np.array(i_list).astype()]
    sorted_smile_list = [tmp[1] for tmp in i_list]
    print(len(sorted_smile_list))
    # store_pkl(sorted_smile_list, "../biosnap_index_smiles_list.pkl")
    store_compressed_pzip(sorted_smile_list, "../biosnap_raw_index_smiles_list.pzip")
